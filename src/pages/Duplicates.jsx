  ...
  return (
    <div className="p-8">
      ...
      {!loading && items.length === 0 && !error && (
        <div className="mt-6 text-sm text-muted">No pending duplicate flags.</div>
      )}
...
function CandidateCard({ app }) {
  return (
    <div className="border border-line rounded-md p-3">
      ...
      <div className="text-xs text-muted mt-0.5">{app.email}</div>
      {app.phone && <div className="text-xs text-muted">{app.phone}</div>}
      <div className="text-xs text-muted mt-1">{app.current_status}</div>
    </div>
  );
}
